...
def write_reports(stats,checks,sample,work,report_path,missing):
    ...
    issues=[
      ...
      ('Medium','Medium','Market-to-book reference is an older intermediate','The stored market-to-book panel ends earlier than the supplied figure sample and stock panel. The report includes raw-ratio and same-historical-source replay differences, so the early-year discrepancies are not certified as harmless vintage noise. Minimum fix: supply the figure sample and stocks from the exact run that wrote dataForFigure8.dta, or a newer dataForFigure8.dta matching the supplied figure sample.'),
    ...
    if diag:
        lines+=['','Historical stock-panel tail counts (the final year is incomplete):','```json',json.dumps(diag['author_stock_year_counts'],indent=2),'```',
                '', 'Acquisition extract audit against the staged final file:', '```json',json.dumps(diag['final_deals_vs_extracts'],indent=2),'```']
    audit_path=work/'crsp_map_audit.json'
    if audit_path.exists():
        lines+=['','Current reconstructed CCM map versus the authors’ dated map (different input vintages):','```json',audit_path.read_text().strip(),'```']
    lines+=['','Numerical regression checks passed for calendar gaps, pooled clipping, depreciation across gaps, intensity timing, missing numerator handling and membership-screen timing. Re-run with `python3 site/figures/check_ports.py`.']
