"""Figure 2 (site): aggregate knowledge capital, organization capital and net PP&E by fiscal year,
...
def compute(s):
    d = s.copy(); d["knowCapital_nom"] = d.G2 * d.cpidef; d["orgCapital_nom"] = d.S2 * d.cpidef; d["ppent_nom"] = d.ppent.fillna(0)
    d = d[d.fyear >= 1977].dropna(subset=["knowCapital_nom", "orgCapital_nom"])
    g = d.groupby("fyear")[["knowCapital_nom", "orgCapital_nom", "ppent_nom"]].sum() / 1000
    g.columns = ["knowledge_bn", "organization_bn", "ppent_bn"]
...
def plot(df):
    fig, ax = new_fig()
    ax.plot(df.fyear, df.ppent_bn, label="Net PP&E", color="#7f8c8d", ls="--")
