...
def compute(sample):
    ...
    if {'intan','ppent','cpidef','intan2'} <= set(d.columns):
    ...
    else:   # the validator passes the authors' panel, which lacks these columns
        d['intensity_net']=np.nan
    d=d[d.fyear>=1977]
    ...
    return out.rename(columns={'intensity_net':'intensity_net_ppe'})   # intensity2 = the paper's gross-PP&E ratio
